...
static void add_pair(const char *k, const char *v) {
    if (cfg_count >= MAX_CFG)
        return;
    strncpy(cfg[cfg_count].key, k, 31);
    cfg[cfg_count].key[31] = '\0';
    strncpy(cfg[cfg_count].val, v, 63);
    cfg[cfg_count].val[63] = '\0';
    cfg_count++;
}
...
void config_load_default(void) {
    ...
    if (!f) {
        fprintf(stderr, "config missing: /etc/aos/config.json\n");
        return;
    }
    char buf[512];
