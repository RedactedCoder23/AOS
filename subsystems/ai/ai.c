...
int ai_setup_wizard(void) {
    ...
    if (!fgets(prov, sizeof(prov), stdin)) return -1;
    prov[strcspn(prov, "\n")] = '\0';
    ...
    p[strcspn(p, "\n")] = '\0';
    if (save_config(prov, p) != 0) return -1;
    return 0;
}

static const char *get_param(void) {
    const char *env = getenv("OPENAI_API_KEY");
    if (!env) env = getenv("AOS_OPENAI_API_KEY");
    if (env) return env;
    if (!cfg_loaded) ai_config_load();
