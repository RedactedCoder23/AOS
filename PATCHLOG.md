...
- `make aos-cli`
- `make test`
- `make run` _(fails: qemu not installed)_
...
- `echo 'ai hello\nexit' | ./build/host_test` _(fails: openai module missing)_
...
- `python3 scripts/desktop_backend.py --help` _(shows usage)_
- `make desktop-ui` _(manual launch)_
...
- `make boot` _(fails: qemu not installed)_
...
- `make branch-net` _(fails: undefined reference to log_message)_
...
- `make test-unit`
- `make test-integration`
- `pre-commit run --files $(git ls-files *.py *.c *.h *.yaml)` _(fails: Username for https://github.com...)_
...
- `clang-tidy` _(errors remain)_
- `pre-commit run` _(failed: GitHub auth prompt)_
- `make test` _(failed: linker cannot find gcc)_
...
- `pre-commit run` _(failed: GitHub auth prompt)_
- `make test-unit` _(fails: duplicate main)_
- `make test-integration` _(fails: linker cannot find gcc)_
...
- `pre-commit run --all-files`
- `make test`
- `clang-tidy src/*.c subsystems/*/*.c` _(fails: no compilation database)_
...
- `make fast-test`
- `./scripts/ci-fast.sh`
- _(docker build skipped: no docker available)_
